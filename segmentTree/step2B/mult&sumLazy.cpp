...
public:
  int size = 1;
  ll no_operation = 1; //change this
  vector<ll> values; //storing the min
  vector<ll> lazy;
  segTree(int n){
    while(size < n) size*= 2;
    values.resize(2*size, 1); //change initial values
    lazy.resize(2*size, 1); //change initial values
  }

  ll operation(ll a, ll b){  //change this
  ...
  ll calc_op(ll a, ll b){ //change this
   return (a + b)%MOD;
  }
  ...
  int calc(int l, int r, int node, int low, int high){
    ...
    if(low>=r || high<=l) return 0; //change this
    if(low>=l && high<=r){
      return values[node];
    }
    int mid = (low + high)>>1;
